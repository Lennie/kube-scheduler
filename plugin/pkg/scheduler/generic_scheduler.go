...
func (g *genericScheduler) selectHost(priorityList api.HostPriorityList) (string, error) {
	if len(priorityList) == 0 {
...
func getBestHosts(list api.HostPriorityList) []string {
	result := []string{}
...
func EqualPriority(_ *api.Pod, podLister algorithm.PodLister, minionLister algorithm.MinionLister) (api.HostPriorityList, error) {
	nodes, err := minionLister.List()
	if err != nil {
		fmt.Errorf("failed to list nodes: %v", err)
		return []api.HostPriority{}, err
	}
